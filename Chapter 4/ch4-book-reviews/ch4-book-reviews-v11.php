        ...
        switch ( $column_name ) {
            case 'book_reviews_author': ?>
                ...
                <div>
                    ...
                    <input type="text" name="book_reviews_author" value="">
                </div>
            <?php break;
            ...
            case 'book_reviews_type': ?>
                <div>
                    ...
                    <?php foreach ($terms as $index => $term) {
                        echo '<option class="book_reviews_type"';
    ...
    if ( !empty( $post ) && 'revision' != $post->post_type ) {
        if ( isset( $_POST['book_reviews_author'] ) ) {
            update_post_meta( $ID, 'book_author', 
              sanitize_text_field( $_POST['book_reviews_author'] ) ); 
        }
 
        if ( isset( $_POST['book_reviews_rating'] ) ) {
            update_post_meta( $ID, 'book_rating', 
                intval( $_POST['book_reviews_rating'] ) ); 
        }
 
        if ( isset( $_POST['book_reviews_type'] ) ) {
            $term = term_exists( 
                        intval( $_POST['book_reviews_type'] ),
                                'book_reviews_book_type' );
            if ( !empty( $term ) ) {
